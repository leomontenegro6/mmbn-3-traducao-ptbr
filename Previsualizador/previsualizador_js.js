...
window.addEventListener("DOMContentLoaded", () => {
  let $textareaScript = $("#script");

  let contents = fs.readFile("script-blue.tpl", "utf8", (err, data) => {
    console.log(err);
    console.log(data);
  });

  let codeMirrorObject = CodeMirror.fromTextArea($textareaScript[0], {
    lineNumbers: true,
    mode: "python",
    theme: "abcdef",
  });
  ...
  codeMirrorObject.on("cursorActivity", function (i, c) {
    ...
    let $spanDialogo = $divPrevia.children("span.dialogo");
    ...
    let range = codeMirrorObject.doc.getRange(
      { line: firstLineSelection, ch: 0 },
      { line: lastLineSelection, ch: 0 }
    );
    const regex = /"""([^\"]*)\"""/gm;
        ...
        if (groupIndex == 1) {
          ...
          if (!match.includes("keyWait") && !match.includes("msgCloseQuick")) {
            for (let i = 0; i < match.length; i++) {
              if (match[i] == "\n") {
                $spanDialogo.append("<br />");
              } else {
                let char = formatarCaractere(match[i]);

                var $spanCaractere = $("<span />")
                  .addClass("caractere " + char)
                  .html("&nbsp;");
                $spanDialogo.append($spanCaractere);
              }
            }
          }
        }
...
function salvarArquivo() {
  let script = codeMirrorObject.doc.getValue();
